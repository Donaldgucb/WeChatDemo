...
//#warning 代码实现自动布局，要设置下面的属性为NO
    ...
    // 创建输入框View
    WCChatView *inputView = [WCChatView inputView];
    ...
    WCAddView *photoView = [WCAddView addView];
    photoView.translatesAutoresizingMaskIntoConstraints = NO;
    [photoView.photoBtn addTarget:self action:@selector(clickPhoto) forControlEvents:UIControlEventTouchUpInside];
    ...
    //addView添加水平约束
    NSArray *addViewHConstraits=[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[photoview]-0-|" options:0 metrics:nil views:views];
    [self.view addConstraints:addViewHConstraits];
    ...
    NSArray *vAddViewConstraits=[NSLayoutConstraint constraintsWithVisualFormat:@"V:[inputview(50)]-0-[photoview(258)]" options:0 metrics:nil views:views];
    [self.view addConstraints:vAddViewConstraits];
...
//    NSLog(@"row==%ld,bodyTpye = %@",indexPath.row,chatType);
    if ([chatType isEqualToString:@"image"]) {
        ...
        if ([msg.outgoing boolValue]) {//自己发
            cell.textLabel.text = [NSString stringWithFormat:@"%@: %@",[WCUserInfo sharedWCUserInfo].user,msg.body];
        }else{//别人发的
...
-(void)addBtnClick{
    ...
    [self.view endEditing:YES];
    self.inputViewBottomConstraint.constant =258;
